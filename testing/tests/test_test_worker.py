...
class TestWorker(TestCase):
    ...
    def test_work_worker_does_not_have_energy_raises(self):
        # Arrange - to arrange data for the function
        w = Worker('Test', 1000, 0)
        self.assertEqual(0, w.money)
        self.assertEqual(0, w.energy)

        # Act -  to test if we can call the work method
        with self.assertRaises(Exception) as ex:
            w.work()

        # Assert - test when energy is =0
        self.assertEqual('Not enough energy.', str(ex.exception))
        self.assertEqual(0, w.money)
        self.assertEqual(0, w.energy)

        # Assert with - values - test when energy is < 0
